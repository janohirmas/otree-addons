...
var lQuestions = js_vars.lQuestions;
...
const myQuestions = lOrdered.concat(lOther);
function processQuestions() {
    let iN = lQuestions.length;
    let iMax = 0;
    while (iN--) {
        let Question = lQuestions[iN];
        if (Question.order === undefined) {
            processItems(Question);
            lOther.push(Question);
            lQuestions.splice(iN,1);
        } else {
            iMax = Math.max(iMax,Question.order)
        }
    }
    iN = 0;
    while (iN<=iMax) {
        for (let x in lQuestions) {
            let Question = lQuestions[x];
            if (Question.order==iN) {
                processItems(Question);
                lOrdered.push(Question);
                break;
            }
        }
        iN++;
...
for (let q in lQuestions) {
    let Question = lQuestions[q];
    // Replace Warnings
    let sMsg = Question.invalidMessage 
    if (typeof sMsg==='string') {
        switch (sMsg) {
            case 'warningAge':
                Question.invalidMessage = warningAge
        }
    }
}
